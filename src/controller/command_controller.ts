...
export const saveCommand = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const controller_id = req.params.controller;
    const device_id = req.params.device;
    if (!controller_id || !device_id) {
      res.status(400).json({ message: "Controller name is required. example <ServerURL>/commands/CTR1/DEV1" });
      return;
    }

    const commandInput = req.body;
    if (!commandInput.value) {
      res.status(400).json({ message: "Bad Request. value are required fields" });
      return;
    }

    await CommandService.saveCommand(controller_id, device_id, commandInput.value);
